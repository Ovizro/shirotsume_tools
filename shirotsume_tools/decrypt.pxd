...
cdef extern from "decrypt.h":
    ...
    cdef void _decrypt "decrypt" (uint8_t* buf, size_t size, uint32_t key) noexcept
    cdef void file_decrypt(uint8_t* buf, size_t size) noexcept
    cdef void decomp(uint8_t* comp_buf, uint8_t* buf, size_t size) noexcept
