...
from .script_patch import find_text_spans, patch_script_file
...
def iter_script_files(script_dir: str | Path) -> list[Path]:
    root = Path(script_dir)
    return sorted(path for path in root.rglob("*.txt") if path.is_file())


def extract_translation_items(script_dir: str | Path, *, source_encoding: str = "cp932", limit: int | None = None) -> list[TranslationItem]:
    root = Path(script_dir)
    items: list[TranslationItem] = []
    for script_path in iter_script_files(root):
        with script_path.open("r", encoding=source_encoding, newline="") as file:
            script = file.read()
...
def chunked(items: Sequence[TranslationItem], size: int) -> Iterable[Sequence[TranslationItem]]:
    for offset in range(0, len(items), size):
        yield items[offset : offset + size]


def chunked_by_budget(items: Sequence[TranslationItem], max_items: int, max_source_chars: int) -> Iterable[Sequence[TranslationItem]]:
    batch: list[TranslationItem] = []
    char_count = 0
    for item in items:
        item_chars = len(item.raw_text)
...
def translate_batch(
    batch: Sequence[TranslationItem],
    *,
    model: str,
    target_language: str,
    memory: dict[str, Any],
...
def sync_translation_csvs(
    ...
    *,
    ...
    for item in items:
        by_file.setdefault(item.file, []).append(item.raw_text)
...
def translate_scripts(
    ...
    *,
    ...
    for item in items:
        existing = existing_by_file.get(item.file)
        if existing is None:
            csv_path = replacements_root / Path(item.file).with_suffix(".csv")
            existing = read_existing_replacement_csv(csv_path)
            existing_by_file[item.file] = existing
        previous = existing.get(item.raw_text)
        if previous is not None and previous[0]:
            existing_translations[item.raw_text] = previous[0]
        else:
    ...
    by_file: dict[str, list[str]] = {}
    for item in items:
        translated = translations.get(item.raw_text)
        if translated is None and item in missing_items:
            translated = translations.get(item.id)
        if translated is None:
            raise ValueError(f"Missing translation for {item.id}")
        translations[item.raw_text] = translated
        by_file.setdefault(item.file, []).append(item.raw_text)
